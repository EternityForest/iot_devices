# iot_devices.host.host

## Classes

| [`DeviceHostContainer`](#iot_devices.host.host.DeviceHostContainer)   | Represents the host's associated state for one device.          |
|-----------------------------------------------------------------------|-----------------------------------------------------------------|
| [`Host`](#iot_devices.host.host.Host)                                 | Represents the host for device plugins, meant to be subclassed. |

## Functions

| [`apply_defaults`](#iot_devices.host.host.apply_defaults)(data, schema)                  |                                                      |
|------------------------------------------------------------------------------------------|------------------------------------------------------|
| [`normalize_legacy_config`](#iot_devices.host.host.normalize_legacy_config)(cls, config) |                                                      |
| [`get_host`](#iot_devices.host.host.get_host)(→ Host)                                    | Get the host that we are runing under, which is just |

## Module Contents

### iot_devices.host.host.apply_defaults(data, schema)
