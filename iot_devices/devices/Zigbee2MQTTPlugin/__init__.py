...
class Zigbee2MQTT(iot_devices.device.Device):
    ...
    def pair(self, t=120):
        ...
        self.connection.publish(
            "zigbee2mqtt/bridge/request/permit_join", {"value": True, "time": t}
        )
    ...
    def __init__(self, name, data, **kw):
        ...
        try:
            ...
            self.connection = mqtt.getConnection(self.config["device.mqtt_server"])
        ...
        if not a == "ZigBee":
            if tn in self.nameToZBInfo:
                ...
                if type == "bool":
                    ...
                    c = hex_to_xy(v)
                    self.connection.publish(
                        topic + "/set", {j["property"]: {"x": c[0], "y": c[1]}}
                    )
                    ...
                    for f in i["definition"]["exposes"]:
                        ...
                        for j in x:
                            if self.config["device.friendly_name"].strip() in (
                                ...
                                tn = "node/" + i["friendly_name"] + "." + j["property"]
                            else:
                                if (
                                    not self.config["device.friendly_name"]
                                    .strip()
                                    .lower()
                            ...
                            if j["name"] == "color_xy" and isALight:
                                self.string_data_point(
                                    "node/" + i["friendly_name"] + "." + j["property"],
                                    subtype="color",
                                    writable=isWritable,
                                )
                                ...
                                def f(t, v, tn=tn, j=j):
                                    if j["property"] in v:
                                        ...
                                        self.set_data_point(tn, v, annotation="ZigBee")
                                ...
                                def f(t, v, tn=tn, j=j):
                                    if j["property"] in v:
                                        self.set_data_point(
                                            tn, v[j["property"]], annotation="ZigBee"
                                        )
                                ...
                                self.numeric_data_point(
                                    tn, min=0, max=1, writable=isWritable
                                )
